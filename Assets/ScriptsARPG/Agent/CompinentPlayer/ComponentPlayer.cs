...
public class ComponentPlayer : MonoBehaviour, IActionHandle
{
    ...
    void LateUpdate()
    {
        ...
        {
            Agent.progressControlTF.position = new Vector3(transform.position.x, transform.position.y + 2.5f, transform.position.z);
        }
    }
    ...
    public void HandleAction(AgentAction a)
    {
        ...
        else if (a is AgentActionInjury)
        {
            ...
            ClearBufferedOrder();
            //GuiManager.Instance.ShowComboProgress(ComboProgress);
            GameScript.Instance.Hits = 0;
            GameScript.Instance.NumberOfInjuries++;
        ...
        else if (a is AgentActionDeath)
        {
            ...
            //GuiManager.Instance.ShowComboProgress(ComboProgress);
            GameScript.Instance.Hits = 0;
            GameScript.Instance.NumberOfDeath++;
    ...
    private void CreateOrderAttack(E_AttackType type)
    {
        ...
        if (CouldAddnewOrder())
        {
            //Debug.Log(" Add order ................"+ order.ToString());
            Agent.BlackBoard.AddOrder(order);
        }
        else
        {
    ...
    private AnimAttackData ProcessCombo(E_AttackType attackType)
    {
        ...
        {// projedem vsechny attacky
           ...
           if (combo.SwordLevel > GameScript.Instance.SwordLevel)
                continue; // nema combo...
            bool valid = ComboProgress.Count <= combo.ComboSteps.Length; // 
            for (int ii = 0; ii < ComboProgress.Count && ii < combo.ComboSteps.Length; ii++)
            {
                if (ComboProgress[ii] != combo.ComboSteps[ii].AttackType || combo.ComboSteps[ii].ComboLevel > GameScript.Instance.ComboLevel[i])
    ...
    private bool NextAttackIsAvailable(E_AttackType attackType)
    {
        ...
        {// projedem vsechny attacky
            ...
            {
                if (progress[ii] != combo.ComboSteps[ii].AttackType || combo.ComboSteps[ii].ComboLevel > GameScript.Instance.ComboLevel[i])
                {
                    valid = false;
                    break;
                }
