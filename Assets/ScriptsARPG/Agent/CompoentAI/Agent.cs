...
public class Agent : MonoBehaviour, IEDable
{
    ...
    {
        AssetsManager.Instance.GetAsset("ProgerssItem.prefab",(GameObject obj, object[] objs) =>
        {
            GameObject _obj = UnityEngine.GameObject.Instantiate(obj, CanvasWorld) as GameObject;
    ...
    public void ReceiveDamage(Agent attacker, E_WeaponType byWeapon, float damage, AnimAttackData data)
    {
        ...
        if (IsKnockedDown)
        {
            ...
            {
                GameScript.Instance.Score += Experience;
                //玩家添加经验，金钱等
            }
            // todo  暴击特效
    ...
    public void ReceiveBlockedHit(Agent attacker, E_WeaponType byWeapon, float damage, AnimAttackData data)
    {
        ...
        else
        {
            ...
            {
                if (attacker.IsPlayer)
                    GameScript.Instance.NumberOfBreakBlocks++;
                BlackBoard.DamageType = E_DamageType.BreakBlock;
            ...
            {
                if (attacker.IsPlayer)
                    GameScript.Instance.NumberOfBlockedHits++;
                BlackBoard.DamageType = E_DamageType.Front;
    ...
    public void ReceiveKnockDown(Agent attacker, Vector3 impuls)
    {
        ...
        {
            GameScript.Instance.Hits += 1;
            GameScript.Instance.NumberOfKnockdowns++;
        }
    ...
    public void ReceiveCriticalHit(Agent attacker, E_CriticalHitType type, bool effectOnly = false)
    {
        ...
        if (attacker.IsPlayer)
        {
            ...
            GameScript.Instance.Score += Experience;
            Player.Instance.AddExperience(Experience, 1.5f + GameScript.Instance.Hits * 0.1f);
            GameScript.Instance.NumberOfCriticals++;
        }
        // todo  暴击特效
