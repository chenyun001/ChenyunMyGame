...
Shader "ASE/Effect/Additive Mask UV Dissolve" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float2 uv0_particleTextureDissolve = i.uv0 * _particleTextureDissolve_ST.xy + _particleTextureDissolve_ST.zw;
                float2 appendResult47 = (float2((_DissolveUSpeed * _Time.y) , (_Time.y * _DissolveVSpeed)));
                float clampResult40 = clamp((i.uv1.x + (pow(tex2D(_particleTextureDissolve, (uv0_particleTextureDissolve + appendResult47)).a , _DissolvePower) * _DissolveIntencity)) , 0.0 , 1.0);
                half3 Emission = ((_TintColor * tex2DNode2 * i.vertexColor * _TextureIntencity) * (_TintColor.a * pow(tex2DNode2.a , _TexturePower) * i.vertexColor.a * (pow(tex2D(_ParticleTextureMask, (uv0_ParticleTextureMask + appendResult31)).a , _MaskPower) * _MaskIntencity)) * clampResult40).rgb;
                half Alpha = 1;
