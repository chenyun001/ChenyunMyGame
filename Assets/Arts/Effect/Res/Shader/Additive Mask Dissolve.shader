...
Shader "ASE/Effect/Additive Mask Dissolve" {
    Properties {
        ...
        _AlphaIntencity("Alpha Intencity", Float) = 1
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float2 uv_ParticleTextureMask = i.uv_texcoord * _ParticleTextureMask_ST.xy + _ParticleTextureMask_ST.zw;
                float2 uv_particleTextureDissolve = i.uv_texcoord * _particleTextureDissolve_ST.xy + _particleTextureDissolve_ST.zw;
                float clampResult22 = clamp((i.uv2_texcoord2.x + (pow(tex2D(_particleTextureDissolve, uv_particleTextureDissolve).a , _DissolvePower) * _DissolveIntencity)) , 0.0 , 1.0);
                half3 Emission = ((_TintColor * tex2DNode2 * i.vertexColor * _TextureIntencity) * (_TintColor.a * pow(tex2DNode2.a , _Texturepower) * i.vertexColor.a * (pow(tex2D(_ParticleTextureMask, uv_ParticleTextureMask).a , _AlphaPower) * _AlphaIntencity)) * clampResult22).rgb;
                half Alpha = 1;
