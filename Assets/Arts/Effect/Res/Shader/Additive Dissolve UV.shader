    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float2 uv0_ParticleColor = i.uv0 * _ParticleColor_ST.xy + _ParticleColor_ST.zw;
                float2 appendResult39 = (float2((_Uspeed * _Time.y) , (_Time.y * _Vspeed)));
                float4 tex2DNode2 = tex2D(_ParticleColor, (uv0_ParticleColor + appendResult39));
                float3 Emission = (_TintColor * tex2DNode2 * i.vertexColor * _TextureIntencity).rgb;
                /*-----------------------º∆À„Alpha----------------------------*/
                float2 uv0_TextureDissolve = i.uv0 * _TextureDissolve_ST.xy + _TextureDissolve_ST.zw;
                float2 appendResult36 = (float2((_MaskUspeed * _Time.y) , (_Time.y * _MaskVspeed)));
                float lerpResult49 = lerp(_softValue , -1.5 , i.uv1.x);
                float clampResult51 = clamp(((tex2D(_TextureDissolve, (uv0_TextureDissolve + appendResult36)).a * _softValue) - lerpResult49) , 0.0 , 1.0);
                half Alpha = (_TintColor.a * (pow(tex2DNode2.a , _Texturepower) * _TextureAlpha) * i.vertexColor.a * clampResult51);
