...
Shader "ASE/Effect/Alpha Blend Mask UV Dissolve" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float2 appendResult31 = (float2((_Uspeed * _Time.y) , (_Time.y * _Vspeed)));
                float4 tex2DNode2 = tex2D(_ParticleColor, (uv0_ParticleColor + appendResult31));
                half3 Emission = (_TintColor * tex2DNode2 * i.vertexColor * _TextureIntencity).rgb;
                float4 temp_cast_1 = (_Texturepower).xxxx;
                float2 uv0_ParticleTextureMask = i.uv_texcoord * _ParticleTextureMask_ST.xy + _ParticleTextureMask_ST.zw;
                float2 appendResult29 = (float2((_MaskUspeed * _Time.y) , (_Time.y * _MaskVspeed)));
                float clampResult52 = clamp((pow(tex2D(_ParticleTextureMask, (uv0_ParticleTextureMask + appendResult29)).a , _MaskPower) * _MaskIntencity) , 0.0 , 2.0);
                float2 uv0_particleTextureDissolve = i.uv_texcoord * _particleTextureDissolve_ST.xy + _particleTextureDissolve_ST.zw;
                float2 appendResult40 = (float2((_DissolveUSpeed * _Time.y) , (_Time.y * _DissolveVSpeed)));
                float clampResult50 = clamp((pow(tex2D(_particleTextureDissolve, (uv0_particleTextureDissolve + appendResult40)).a , _DissolvePower) * _DissolveIntencity) , 0.0 , 1.0);
                float clampResult51 = clamp((i.uv2_texcoord2.x + clampResult50) , 0.0 , 1.0);
                half Alpha = (_TintColor.a * (pow(tex2DNode2 , temp_cast_1) * _TextureAlpha) * i.vertexColor.a * clampResult52 * clampResult51).r;
