...
Shader "ASE/Effect/Additive Mask UV One Dissolve" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float2 appendResult34 = (float2(i.uv1.x , i.uv1.y));
                float2 temp_output_36_0 = (uv0_ParticleTexture + appendResult34);
                float4 tex2DNode2 = tex2D(_ParticleTexture, temp_output_36_0);
                float2 appendResult31 = (float2((_MaskUspeed * _Time.y) , (_Time.y * _MaskVspeed)));
                float lerpResult57 = lerp(_Soft , -1.5 , i.uv1.z);
                float clampResult59 = clamp(((tex2D(_TextureSample0, temp_output_36_0).a * _Soft) - lerpResult57) , 0.0 , 1.0);
                half3 Emission = ((_TintColor * tex2DNode2 * i.vertexColor * _TextureIntencity) * (_TintColor.a * pow(tex2DNode2.a , _Texturepower) * i.vertexColor.a * (pow(tex2D(_ParticleTextureMask, (i.uv0 + appendResult31)).a , _Maskpower) * _MaskIntencity) * clampResult59)).rgb;
                half Alpha = 1;
