...
Shader "ASE/Effect/Additive Mask Noise Dissolve UV" {
    Properties {
        ...
        _NoiseIntencity("Noise Intencity", Float) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            struct VertexOutput {
                ...
                float2 uv2_texcoord2 : TEXCOORD0;
                float4 vertexColor : COLOR;
            };
            VertexOutput vert (VertexInput v) {
                VertexOutput o = (VertexOutput)0;
                o.uv0 = v.texcoord0;
                o.uv2_texcoord2 = v.texcoord0;
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float2 uv0_TextureNoise = i.uv0 * _TextureNoise_ST.xy + _TextureNoise_ST.zw;
                float2 appendResult38 = (float2((_Noiseuspeed * _Time.y) , (_Time.y * _NoisevSpeed)));
                float4 tex2DNode2 = tex2D(_ParticleTexture, ((uv0_ParticleTexture + appendResult23) + (tex2D(_TextureNoise, (uv0_TextureNoise + appendResult38)).a * _NoiseIntencity)));
                float2 uv_particleTextureDissolve = i.uv0 * _particleTextureDissolve_ST.xy + _particleTextureDissolve_ST.zw;
                float clampResult53 = clamp((i.uv2_texcoord2.x + (pow(tex2D(_particleTextureDissolve, uv_particleTextureDissolve).a , _DissolvePower) * _DissolveIntencity)) , 0.0 , 1.0);
                half3 Emission = ((_TintColor * tex2DNode2 * i.vertexColor * _TextureIntencity) * (_TintColor.a * pow(tex2DNode2.a , _TexturePower) * i.vertexColor.a * clampResult53)).rgb;
                half Alpha = 1;
