...
Shader "ASE/Effect/Additive Mask" {
    Properties {
        _TintColor("Tint Color", Color) = (1,1,1,1)
        _ParticleTexture("Particle Texture", 2D) = "white" {}
        _TxetureIntencity("Txeture Intencity", Float) = 1
    ...
    SubShader {
        ...
        Pass {
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                uniform float _TxetureIntencity;
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                float2 uv_ParticleTexture = i.uv0 * _ParticleTexture_ST.xy + _ParticleTexture_ST.zw;
                float4 tex2DNode2 = tex2D(_ParticleTexture, uv_ParticleTexture);
                float2 uv_ParticleTextureMask = i.uv0 * _ParticleTextureMask_ST.xy + _ParticleTextureMask_ST.zw;
                half3 Emission = ((_TintColor * tex2DNode2 * i.vertexColor * _TxetureIntencity) * (_TintColor.a * pow(tex2DNode2.a , _Texturepower) * i.vertexColor.a * (pow(tex2D(_ParticleTextureMask, uv_ParticleTextureMask).a , _MaskPower) * _MaskIntencity))).rgb;
                half Alpha = 1;
